...
public:
  ...
  /** Search for the baudrate the module is workint at, and set it as
      the starting baudrate.

      @return The found baudrate, -1 if no baudrate was found to be
      working.
   */
  int autobaud();
  ...
  /** Start the NB IoT modem, attaching to the NB IoT or LTE Cat M1 network
    ...
    */
  NB_NetworkStatus_t begin(const char* pin = 0, bool restart = true, bool synchronous = true, bool softreset = false);
  NB_NetworkStatus_t begin(const char* pin, const char* apn, bool restart = true, bool synchronous = true, bool softreset = false);
  NB_NetworkStatus_t begin(const char* pin, const char* apn, const char* username, const char* password, bool restart = true, bool synchronous = true, bool softreset = false);
  ...
  /** Weidos initial config URAT = 8 and CPSMS = 0
  ...
  bool getStates(const char* statePrefix, String* response); 
