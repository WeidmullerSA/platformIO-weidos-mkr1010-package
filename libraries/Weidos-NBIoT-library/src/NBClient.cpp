...
enum {
  ...
  CLIENT_STATE_MANAGE_SSL_PROFILE_2,
  CLIENT_STATE_WAIT_MANAGE_SSL_PROFILE_RESPONSE_2,
  CLIENT_STATE_MANAGE_SSL_PROFILE_3,
  CLIENT_STATE_WAIT_MANAGE_SSL_PROFILE_RESPONSE_3,
...
int NBClient::ready()
{
  ...
  switch (_state) {
    ...
    case CLIENT_STATE_ENABLE_SSL: {
      MODEM.sendf("AT+USOSEC=%d,1,1", _socket);
    ...
    case CLIENT_STATE_MANAGE_SSL_PROFILE_2: {      
      MODEM.send("AT+USECPRF=1,2,0");

      _state = CLIENT_STATE_WAIT_MANAGE_SSL_PROFILE_RESPONSE_2;
      ready = 0;
      break;
    }

    case CLIENT_STATE_MANAGE_SSL_PROFILE_3: {
      MODEM.sendf("AT+USECPRF=1,10,\"%s\"", _host);
      _state = CLIENT_STATE_WAIT_MANAGE_SSL_PROFILE_RESPONSE_3;
      ready = 0;
      break;
    }
    ...
    case CLIENT_STATE_WAIT_MANAGE_SSL_PROFILE_RESPONSE: {
      if (ready > 1) {
        ...
        _state = CLIENT_STATE_MANAGE_SSL_PROFILE_2;
      }
      ready = 0;
      break;
    }

    case CLIENT_STATE_WAIT_MANAGE_SSL_PROFILE_RESPONSE_2: {
      if (ready > 1) {
        _state = CLIENT_STATE_CLOSE_SOCKET;
      } else {
        _state = CLIENT_STATE_MANAGE_SSL_PROFILE_3;
      }
      ready = 0;
      break;
    }

    case CLIENT_STATE_WAIT_MANAGE_SSL_PROFILE_RESPONSE_3: {
      if (ready > 1) {
        _state = CLIENT_STATE_CLOSE_SOCKET;
      } else {
